...
'https://billing.stripe.com/p/login/test_9B600l7R2bv561g11tcEw00'

export function AccountPane() {
    const { user, updateUser } = useInfo();
    const { injectHeader } = useAuth();
    
    const [ name, setName ] = useInputState(user?.name || '');
    const [ phone, setPhone ] = useInputState(user?.phone || '');
    ...
    return (<>
        ...
        <Container>
            ...
            <Text c={'dimmed'}>Your User ID: <Text span c='purple' size='1.2rem'>{user.id}</Text> can be handing sending with support messages</Text>
            <TextInput 
                label='Email'
                readOnly
                value={user?.email}
            ...
            <Button fullWidth onClick={startSubscription}>
