...
export function QrPane() {
    ...
    const createNewQr = () => {
        return (<Stack h={'100%'} align="center" justify="center" py="md">
            <Text mb={'-14px'}>Create New QR</Text>
            <PlusIcon size={'2rem'} />
            <TextInput w={{ base: '100%', sm: 'auto' }} label='Link to' placeholder="https://example.com" value={newQrLink} onChange={setNewQrLink} />
            <TextInput w={{ base: '100%', sm: 'auto' }} label='Nickname' placeholder="QR to Homepage" value={newQrNickname} onChange={setNewQrNickname} />
    ...
    const qrListView = () => qrList.map(q => {
        ...
        return (<div key={q.id}>
            <Group justify='space-between' wrap='nowrap' id={q.id}>
                <Stack gap={0} align="start" ta={'start'} style={{ minWidth: 0, flex: 1 }}>
                    <Text size='lg' truncate={'end'} w="100%">{firstLine}</Text>
                    <Text size='md' c='dimmed' truncate={'end'} w="100%">{q.redirectLink}</Text>
                </Stack>
                <Group gap={12} wrap="nowrap" justify='right'>
                    <Text c={q.active ? 'green' : 'orange'}>{q.active ? 'ON' : 'OFF'}</Text>
                    <Button size="xs" onClick={() => selectQrObj(q.id)} variant={selectedQr?.id === q.id ? 'filled' : 'outline'}>Select</Button>
                </Group>
            </Group>
            <Divider ml={{ base: 0, sm: '2rem' }} size={'sm'} mt="xs" />
        </div>)
    })

    const creditInfo = () => {
        return (<Group justify='space-between' wrap="nowrap" mt={'12px'} mb={'8px'} gap="xs">
            <Stack ta='center' gap={0}>
                <Text c='dimmed' size="sm">Total QRs</Text>
                <Text size={'1.4rem'}>{qrList.length}</Text>
            </Stack>
            <Stack ta='center' gap={0}>
                <Text c='dimmed' size="sm">Credits (used / owned)</Text>
                <Text size="1.4rem">{usedCredits} / {ownedCredits}</Text>
                {isMonthlySuber && <Text c='yellow' size="xs">Monthly Subscriber</Text>}
            </Stack>
    ...
    return (<Grid py='1rem' gap={{ base: 'sm', md: 'md' }}>
